...
- Phase判断はPhase統括
- コスト記録はAI経理本人
- `#Progress`はチャットツールとして扱う。本文は原則1〜2文で、重要な状態変化と次の行動だけを書く
- HEAD / hash / branch名 / test件数 / file数 / command / 詳細な証拠は、状態理解に不可欠な場合を除き`#Progress`には書かない。詳細はIssue / handoff / commitへ残す
