...
- **Phase統括** — 全体判断・進捗整理
- **Codex Sol** — 主実装担当。通常の実装、fix、統合、handoffを担当
- **Codex Astra** — 高難度実装担当。複雑なバグ、難しい原因特定・recovery、Solでは重い高難度taskを担当
- **Codex Luna** — ファイル・文書整理担当。docs、handoff、命名、ファイル整理などを担当し、コードロジック変更や仕様判断は原則担当しない
- **Work QA** — 独立レビュー担当
- **AI経理** — クレジット・コスト管理
- **オーナー** — 最終判断・必要な実機操作

旧Codex Aは過去ログ互換としてのみ残し、新規記録では使用しない。
...
現在進行中のPhase専用ログ。対象PhaseとGitHub Issueは `phase-manifest.json` から決まる。
...
代理投稿でもactorは実際の投稿者ではなく、その状態の主体で決める。Codexは実担当に合わせてCodex Sol / Codex Astra / Codex Luna、WorkはWork QAを使う。
...
- **Codex Sol / Codex Astra / Codex Luna** — `START / FIX / PASS / BLOCKED`
- **Work QA** — `PASS / FAIL`
- **Phase統括** — `NEXT / CLOSED / BLOCKED`
- **AI経理** — `COST`
- `WAITING` は役割共通の例外statusとし、単なる次工程待ちには使わない

### Progressの標準順序

新しい作業単位は、原則として次の順序で記録する。

1. Phase統括 NEXT — 対象taskと担当を決め、作業開始前にprogressを更新する
2. Codex担当 — 必要ならSTART、完了時はPASS、finding修正時はFIX
3. Work QA — 独立レビュー結果をPASSまたはFAILで記録する
4. Phase統括 — Work結果を受けてCLOSED、BLOCKED、または次のNEXTを判断する

AI経理COSTは対応する節目の直後に入れてよいが、この工程順序を置き換えない。

Codex PASSを先に投稿してからPhase統括NEXTを後付けしない。記録漏れがあった場合も過去時刻を作らず、現在時刻で補正理由を明示する。

Phase統括がNEXTを記録するときは、原則として同じコメントで `progress: xx%` を更新する。進捗率の決定はPhase統括だけが行い、Codex / Workの代理投稿には入れない。

PASSとCLOSEDは別物。Codex PASSだけでcheckpointを閉じず、Work QAの独立確認後にPhase統括がclosureを判断する。

完了コメントの本文末には次は〜を短く入れ、次工程が分かるようにする。

本文は短い自然な日本語にする。task名、状態、actor名を引用符、バッククォート、装飾的な括弧で囲まない。正式なactorとstatusは先頭metadataで表し、本文では同じstatusを機械的に繰り返さない。
...
- ツール利用不可と述べる前に実際の利用可否を確認し、実装許可がないだけなら未反映と表現する
...
これらは通常の適切な役割分担であり、単独ではオーナー介入コストの悪化と扱わない。Phase統括 / Codexは、**前提・押す場所・実行内容・返してほしい結果を最小手順で指定**する。
...
実機由来のfixをレビューする場合、Work QAはオーナーが最終的にPASSしただけで閉じず、原因が全再現経路を説明していること、修正前FAIL→修正後PASSのregressionがあること、実機反復が原因証明の代わりに使われていないことも確認する。

また、必要なOAuth / login / GAS関数実行 / 実機操作をオーナーへ任せたこと自体はfindingにしない。見るべきなのは、その前後でCodexのPASS品質が十分だったか、不要な出戻りを発生させていないかである。

## 不可逆操作の安全原則

**同じ目的を達成できるなら、不可逆操作より可逆操作を優先する。**

- 操作前に、その操作が可逆か不可逆かを確認する
- close / reopen、archive、disable、revertなどの可逆な代替手段がある場合は、delete / purge / force / reset / overwrite / revokeより優先する
- 不可逆操作、または番号再利用不可・履歴消失・データ消去・権限喪失などの永続的副作用がある操作は、実行前に**復元可否と副作用をオーナーへ説明し、明示承認を得る**
- 不可逆性や副作用が不明な場合は、確認が終わるまで実行しない
- オーナーへ不可逆操作の手順を案内する場合も、先に可逆な代替手段と不可逆性を説明する
- Git履歴から容易に復元できる一時helper / workflow削除など、事前承認済み範囲内の通常cleanupは従来どおり自律的に完了してよい
...
- 不可逆操作より可逆操作を優先し、不可逆または永続的副作用がある場合は復元可否と副作用の説明後にオーナーの明示承認を得る
