...
- 初回FULLの後は、原則として **変更差分 + 未解決finding + それを閉じるのに必要な証拠だけ**をWorkへ渡す。
- 既にPASS済みで、関連変更によりinvalidateされていない範囲は再レビューさせない。
- closure / targeted reviewでは、Phase 1の **12/2前後**の消費感を目安にする。
- targeted reviewで **28/4級**の消費が続く場合は、Workの性能不足ではなく、まずPhase統括のレビュー範囲・依頼文が広すぎないかを改善対象とする。
- Workの独立性は維持し、必要なfinding確認や証拠まで削らない。削るのは既に閉じた範囲・無関係な探索・再確認だけ。
